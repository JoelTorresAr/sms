                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <router-link  :to="{name:'tipomedia.crear'}">
                                    Crear Tipomedia
                                </router-link>
                            </li>
                        </ul>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="Nombre" placeholder="Nombre" v-model="record.name" @keyup.enter="setRegistrarTipomedia">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-12">
                                    <button type="button" class="btn btn-outline-primary has-ripple" @click.prevent="setRegistrarTipomedia" v-loading.fullscreen.lock="fullscreenLoading">Guardar</button>
...
<script>
    export default {
        ...
        methods: {
            ...
            async setRegistrarTipomedia(){
                if (this.validarRegistrarTipomedia()) {
                    return;
                }

                this.fullscreenLoading = true;

                var url = '/configuracion/tipomedia/store'

                const res = await this.callApi('post', url, this.record)

                if (res.status == 200) {
                    this.notification('success', 'Registro creado con éxito', 0)
                    this.$router.push('/tipomedia');
                } else {
                    if (res.status == 422) {
